...
export class SbarStore {
  ...
  loadTransfers(): void {
    this.patientApi.getAll().pipe(
      switchMap(patientResponses => {
        ...
        return forkJoin(
          patients.map(patient =>
            this.api.getByPatientId(patient.id).pipe(
              map(handovers => SbarAssembler.toEntityList(handovers, id =>
                patients.find(p => p.id === id)?.fullName ?? `Paciente #${id}`
              ))
            )
          )
      ...
      this._transfers.set(
        transfers.sort((a, b) => b.transferredAt.getTime() - a.transferredAt.getTime())
      );
    });
  }
  ...
  registerTransfer(form: SbarForm): void {
    ...
    this.api.register(request).pipe(
      ...
      this.audit.register(AuditAction.SBAR_TRANSFER, `Registró traspaso SBAR para ${transfer.patientName} enviado a ${receiverName} por ${currentUser?.fullName ?? 'Sistema'}`);
    });
  }

  acknowledgeTransfer(transferId: string, incomingNurseId: string, notes = 'Traspaso SBAR revisado y atendido.'): void {
    this.api.acknowledge(transferId, {
      incomingNurseId: Number(incomingNurseId),
      additionalNotes: notes,
    }).subscribe(updated => {
      const transfer = SbarAssembler.toEntity(updated, this.resolvePatientName(String(updated.patientId)));
      this._transfers.update(list => list.map(item => item.id === transfer.id ? transfer : item));
      this.audit.register(AuditAction.SBAR_TRANSFER, `Atendió traspaso SBAR para ${transfer.patientName}`);
    });
  }

  private resolvePatientName(patientId: string): string {
    return this.patients.patients().find(patient => patient.id === patientId)?.fullName ?? `Paciente #${patientId}`;
  }
