...
export class PatientMonitoringComponent implements OnInit {
  ...
  protected patientVitals = computed(() => this.vitalSignStore.vitalSigns().filter(item => item.patientId === this.patient()?.id));
  protected patientEvents = computed(() => this.eventStore.events().filter(item => item.patientId === this.patient()?.id));
  ...
  ngOnInit(): void {
    ...
    this.patientApi.getById(id).subscribe(found => this.patient.set(PatientAssembler.toEntity(found)));
  }
