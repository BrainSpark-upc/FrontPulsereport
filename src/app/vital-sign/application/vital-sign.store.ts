...
export class VitalSignStore {
  ...
  loadVitalSigns(): void {
    this.api.getAll().subscribe(res => {
      const signs = VitalSignAssembler.toEntityList(res, id => this.resolvePatientName(id))
  ...
  recordVitalSign(form: VitalSignForm): void {
    ...
    const numericNurseId = Number(currentUser?.id);
    const nurseId = Number.isFinite(numericNurseId) && numericNurseId > 0 ? numericNurseId : 1;
    ...
    this.api.record(request).subscribe(created => {
      const sign = VitalSignAssembler.toEntity(created, this.resolvePatientName(String(created.patientId)));

      this._vitalSigns.update(list => [sign, ...list]);
      this.audit.register(AuditAction.VITAL_SIGN_RECORDED, `Registró signos vitales de ${sign.patientName} con riesgo ${sign.riskLabel}`);
  ...
  private createAlertsWhenNeeded(sign: VitalSign): void {
    ...
    const severity = sign.riskLevel === RiskLevel.CRITICAL ? AlertSeverity.CRITICAL : AlertSeverity.HIGH;
  ...
  private getClinicalFindings(sign: VitalSign): string[] {
    ...
    return findings.length ? findings : [`Riesgo ${sign.riskLabel} detectado en monitoreo clínico.`];
  }
