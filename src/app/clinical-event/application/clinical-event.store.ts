...
export class ClinicalEventStore {
    ...
    loadEvents(): void {
        this.api.getAll().subscribe(res => {
            this._events.set(ClinicalEventAssembler.toEntityList(res).sort((a, b) => b.occurredAt.getTime() - a.occurredAt.getTime()));
        });
    }
    ...
    registerEvent(form: Omit<ClinicalEventResponse, 'id' | 'patientName' | 'nurseId' | 'nurseName' | 'occurredAt'>): void {
        ...
        this.api.register(request).subscribe(created => {
            ...
            this.audit.register(AuditAction.CLINICAL_EVENT_REGISTERED, `Registró evento clínico ${event.severity} en ${event.patientName}: ${event.title}`);
            this.audit.register(AuditAction.BUSINESS_TRANSACTION_EXECUTED, `Transacción: evento clínico → alerta si aplica → auditoría para ${event.patientName}`);
    ...
    private createAlertWhenNeeded(event: ClinicalEvent): void {
        ...
        this.notifications.createClinicalAlert({
            ...
            severity: event.severity === ClinicalEventSeverity.CRITICAL ? AlertSeverity.CRITICAL : AlertSeverity.MODERATE,
