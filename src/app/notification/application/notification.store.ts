...
export class NotificationStore {
  ...
  loadAlerts(): void {
    this.api.getAll().subscribe(res => {
      const alerts = AlertAssembler.toEntityList(res, id => this.resolvePatientName(id))
  ...
  createManualAlert(request: ManualAlertPayload): void {
    ...
    const payload = {
      ...
      triggeredBy: currentUser?.fullName ?? 'Sistema',
    };

    this.api.create(payload).subscribe(created => {
      const alert = AlertAssembler.toEntity(created, this.resolvePatientName(String(created.patientId)));
      this._alerts.update(list => [alert, ...list]);
      this.audit.register(AuditAction.ALERT_CREATED, `Creó alerta para ${alert.patientName}: ${alert.title}`);
    });
  }
  ...
  createClinicalAlert(request: ClinicalAlertPayload): void {
    ...
    this.api.create(payload).subscribe(created => {
      const alert = AlertAssembler.toEntity(created, request.patientName ?? this.resolvePatientName(String(created.patientId)));
      this._alerts.update(list => [alert, ...list]);
      this.audit.register(AuditAction.ALERT_CREATED, `Alerta generada para ${alert.patientName}: ${alert.title}`);
    });
  }
  ...
  acknowledge(alertId: string): void {
    ...
    this.api.acknowledge(alertId, currentUser?.fullName ?? 'Sistema').subscribe(updated => {
      const alert = AlertAssembler.toEntity(updated, this.resolvePatientName(String(updated.patientId)));
      this._alerts.update(list => list.map(a => a.id === alert.id ? alert : a));
      this.audit.register(AuditAction.ALERT_ACKNOWLEDGED, `Atendió alerta de ${alert.patientName}: ${alert.title}`);
    });
  }
  ...
  resolve(alertId: string): void {
    ...
    this.api.resolve(alertId, currentUser?.fullName ?? 'Sistema').subscribe(updated => {
      const alert = AlertAssembler.toEntity(updated, this.resolvePatientName(String(updated.patientId)));
      this._alerts.update(list => list.map(a => a.id === alert.id ? alert : a));
      this.audit.register(AuditAction.ALERT_RESOLVED, `Cerró alerta de ${alert.patientName}: ${alert.title}`);
    });
  }

  filterBy(status: 'Todas' | 'Críticas' | 'Moderadas'): Alert[] {
    const alerts = this._alerts().filter(a => a.status !== AlertStatus.CLOSED);
  ...
  private resolvePatientName(patientId: string): string {
    return this.patientStore.patients().find(patient => patient.id === patientId)?.fullName ?? `Paciente #${patientId}`;
  }
